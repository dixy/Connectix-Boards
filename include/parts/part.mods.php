...
if (isset($_POST['punish']))
{
	if (isset($_POST['selectuser_type']))
	{
		...
		elseif ($_POST['selectuser_type'] == 'name' && isset($_POST['selectuser_name']) && $user_i = getUserId($_POST['selectuser_name']))
		{
			...
			trigger_error(lang('mod_punish_nouser'), E_USER_WARNING);
		
		if ($user_i)
			redirect(manage_url('index.php?act=mods&page=2', 'forum-moderators.html?page=2').'&punish='.$user_i);
	}
		...
		if (isUser((int)$_GET['punish']))
		{
			if (is_numeric($_POST['punishtime']))
			{
				...
				$GLOBALS['cb_db']->query("UPDATE ".$GLOBALS['cb_db']->prefix."users ".
					...
					"SET usr_punished = '".$punish."' ".
					"WHERE usr_id = ".(int)$_GET['punish']." AND gr_status = 0");
				...
				if ($GLOBALS['cb_db']->affected_rows() == 1)
				{
					...
					trigger_error(lang('mod_punish_cannot'), E_USER_WARNING);
			}
			else
				trigger_error(lang('mod_punish_badtime'), E_USER_WARNING);
		}
	}
}
elseif (isset($_POST['note'], $_POST['sendnote']))
{
	if (isUser((int)$_GET['punish']))
	{
		...
		{
			$GLOBALS['cb_db']->query('INSERT INTO '.$GLOBALS['cb_db']->prefix.'modnotes (mn_modid,mn_userid,mn_date,mn_note) '.
				'VALUES ('.$_SESSION['cb_user']->userid.','.(int)$_GET['punish'].','.time().',\''.clean($_POST['note'],STR_MULTILINE).'\')');
	...
	if (isset($_GET['punish']) && is_numeric($_GET['punish']))
	{
		...
		if ($usr = $GLOBALS['cb_db']->fetch_assoc($q))
		{
			...
			if (isset($pun[1], $pun[2]) && $pun[1] + $pun[2] > time())
			{
				...
				$GLOBALS['cb_tpl']->assign('m_moduser_punished', FALSE);
			...
			{
				$notes[] = array(
					'user' => '<a href="'.manage_url('index.php?act=user&amp;showprofile='.$data['mn_modid'],'forum-m'.$data['mn_modid'].','.rewrite_words($data['usr_name']).'.html').'">'.$data['usr_name'].'</a>',
					'date' => dateFormat($data['mn_date']),
					'note' => $data['mn_note']);
				$dates[] = $data['mn_date'];
			}
			...
			while ($data = $GLOBALS['cb_db']->fetch_assoc($ret))
			{
				...
				$notes[] = array(
					'user' => '<a href="'.manage_url('index.php?act=user&amp;showprofile='.$data['log_usermake'],'forum-m'.$data['log_usermake'].','.rewrite_words($data['usr_name']).'.html').'">'.$data['usr_name'].'</a>' ,
					'date' => dateFormat($data['log_timestamp']),
					'mod' => lang(getLogDesc($data['log_type'])).$param);
			...
			redirect(manage_url('index.php?act=mods&page=2', 'forum-moderators.html?page=2'));
	}
